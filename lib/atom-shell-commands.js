( function( window, atom, require, module, undefined ) {
	...
	function getEnv() {
		...
		var size = filepath.length - filename.length - 1;
		var filedir = filepath.substring(0, (size >= 0)? size : 0);
		var paths = atom.project.relativizePath(filepath);
		
		var names = filename.split('.');
		var fileext = '.' + names[names.length - 1];
		var size = filename.length - fileext.length;
		var filemain = filename.substring(0, (size >= 0)? size : 0);

		var env = {
			AtomFilePath: filepath,
			AtomFileName: filename,
			AtomFileDir: filedir,
			AtomFileExtName: fileext,
			AtomFileNameWithoutExt: filemain,
			AtomProjectDir: paths[0],
			AtomProjectRel: paths[1]
		};
		return env;
	}
