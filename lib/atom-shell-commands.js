( function( window, atom, require, module, undefined ) {
	...
	commands.activate = function( state ) {
		...
		var cmdstop = atom.commands.add('atom-workspace', prefix + 'stop', function () {
			childStop();
		});
		
		commands.subscriptions.add(cmdstop);
		
		var cmdkill = atom.commands.add('atom-workspace', prefix + 'kill', function () {
			childKill();
		});
		
		commands.subscriptions.add(cmdkill);
	}
	...
	function execute( command, args, options, matchs ) {
		...
		// Announcing launch
		var text = "> " + command + ' ' + JSON.stringify( args ) + ' cwd="' + cwd + '"';
		...
		proc.on('close', function(code) {
			...
			if (code == null || code == undefined) {
				output('[Killed in ' + delta.toFixed(2) + ' seconds]', 'echo');				
			}	else {
				output('[Exited with code ' + code.toString() + ' in ' + delta.toFixed(2) + ' seconds]', 'echo');
			}
