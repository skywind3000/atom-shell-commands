( function( window, atom, require, module, undefined ) {
	...
	function quickfixPush(view, filename, line, column, position, style1, style2) {
		var obj = {
			view: view,
			filename: filename,
			line: line,
	...
	function quickfixActive(index) {
		...
		if (quickfix.index >= 0 && quickfix.index < quickfix.error.length) {
			...
			var view = error.view;
			if (view) {
				view.position.removeClass(error.style2);
				view.position.addClass(error.style1);
			}
			quickfix.index = -1;
		}
		...
		if (error.view) {
			error.view.position.removeClass(error.style1);
			error.view.position.addClass(error.style2);
		}
		...
		function output(text, style) {
			for (var i = 0; i < matchs.length; i++) {
				...
				if (text) {
					...
					quickfixPush(text, file, line, col, position, style, style + '-selected');
				}
				return;
			}
